...
public class Vision extends Subsystem {
        ...
        try {
			...
			for (int i = 0; i < verticalTargetCount; i++)
			{
				...
				for (int j = 0; j < horizontalTargetCount; j++)
				{
                                ...
                                //Determine if Hot is on the left or the right
                                target.isLeftSide = isLeftHot(target);
                                target.isRightSide = isRightHot(target);
                            ...
                            if(verticalTargetCount > 0)
                            {
                                    ...
                                    if(target.Hot)
                                    {
                                            ...
                                            if(target.isLeftSide)
                                            {
                                                System.out.println("Shoot left");
                                            }
                                            else if (target.isRightSide)
                                            {
                                                System.out.println("Shoot right");
                                            }
                                            else
                                            {
                                                System.out.println("Don't Shoot, Error");
                                            }
    ...
    public void clearUnusedData(BinaryImage thresholdImage,BinaryImage filteredImage,ColorImage image ){
        try {
            filteredImage.free();
            thresholdImage.free();
            image.free();
	...
	boolean hotOrNot(TargetReport target)
	{
        ...
        boolean isRightHot(TargetReport target){
            
            return target.rightScore > LR_SCORE_LIMIT;
        }
        boolean isLeftHot(TargetReport target){
            return target.leftScore > LR_SCORE_LIMIT;
        }
}
